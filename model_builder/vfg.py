...
class VFGNode:
    def __init__(self, node: Node):
        self.name = node.name
        info_json: dict[str, str] = json.loads(node.label.encode("utf-8").decode("unicode_escape").replace("\\<", "<"))
        self.info = PATTERNS["NodeLabelSeparation"].split(node.label.removeprefix("{").removesuffix("}"))

        self.type = info_json["type"].removesuffix("VFGNode")
        self.id = int(info_json["id"])
        self.t = SHORT_TYPE[self.type]
        self.ir = info_json.get("ir", "")
        self.function = info_json.get("f", "")
        self.basic_block = info_json.get("bb", "")
    ...
    def demangle(mangled_name: str) -> str:
        return (
            subprocess.run(["c++filt", "-p", mangled_name], capture_output=True, text=True, check=True)
            .stdout.strip()
            .replace("<", "\\<")
            .replace(">", "\\>")
        )
...
class VFG:
    def __init__(self, graph: Graph) -> None:
        self.nodes: dict[str, VFGNode] = {
            node.name: VFGNode(node) for node in tqdm(graph.nodes, bar_format="{l_bar}{bar:50}{r_bar}", unit=" nodes")
        }
    ...
    def search_nodes(self, type: str, function: str, basic_block: str) -> set[VFGNode]:
        ...
        for node in self:
            if node.type == type and node.function == function and node.basic_block == basic_block:
