...
class VFGNode:
    def __init__(self, name: str, label: str = ''):
        ...
        self._ir = self._info[2] if self._info[2] != '(none)' else ''
...
class VFG:
    ...
    def __init__(self, nodes: Optional[dict[str, VFGNode]] = None, edges: Optional[set[VFGEdge]] = None) -> None:
        self._nodes = nodes if nodes is not None else dict()
        self._edges = edges if edges is not None else set()

    @classmethod
    def from_dot_file(cls, dot_file: str) -> VFG:

        nodes: dict[str, VFGNode] = dict()
        ...
        with open(dot_file, mode='r', encoding="utf-8") as file:
            for line in file:
                ...
                if node_match:
                    ...
                    nodes[node_name] = VFGNode(node_name, node_info)
...
class Model:
    ...
    def __init__(self, graph: VFG, starting_node_ids: list[int]) -> None:
        self._model = graph
        self._starting_node_ids = starting_node_ids

    @property
    def starting_node_ids(self):
        """Wrap the ids to make it readonly."""
        return self._starting_node_ids.copy()

    @staticmethod
    def transform_node(node: VFGNode) -> None:
        ...
        match node.type:
            ...
            case 'GepVFGNode':
                ...
                assert node.lower_node_number == 1
    ...
    @classmethod
    def connect_actual_param_ret_nodes(cls, vfg: VFG, node: VFGNode) -> bool:
        assert node.type == 'ActualParmVFGNode'
        if node.function is None or node.basic_block is None:
            cls.logger.warning('ActualParmVFGNode (%d) does not contains necessary information', node.id)
            return False
        updated = False
        callsite_nodes = vfg.search_nodes('ActualRetVFGNode', node.function, node.basic_block)
        for callsite_node in callsite_nodes:
            if callsite_node.ir is None:
                cls.logger.warning('ActualRetVFGNode (%d) does not contains necessary information', callsite_node.id)
                continue
            if node.label in callsite_node.ir and not node.has_edge(node.name, callsite_node.name):
                vfg.add_edge(VFGEdge(node.name, callsite_node.name))
                cls.logger.debug("VFG updated when processing ActualParmVFGNode (%d)", node.id)
                updated = True
        return updated

    @classmethod
    def extract(cls, vfg: VFG, starting_node_ids: list[int]) -> Model:
        """Extract model from vfg."""
        cls.logger.debug("Start to extract model")
        visited = set()
        iter_times = 0
        while True:
            iter_times += 1
            cls.logger.info("Transform iteration: %d", iter_times)
            subvfg = vfg.get_subgraph(starting_node_ids)
            cls.logger.info("SUB VFG scale: (node: %d, edge: %d)",
                            subvfg.node_number, subvfg.edge_number)

            for i, node in enumerate(subvfg):
                cls.logger.debug("Transforming %d/%d", i + 1, subvfg.node_number)
                if node not in visited:
                    cls.transform_node(node)
                    visited.add(node)
                if node.type == 'ActualParmVFGNode':
                    cls.connect_actual_param_ret_nodes(vfg, node)
            break

        return cls(subvfg, starting_node_ids)
    ...
    #                     if node.function is None or node.basic_block is None:
    #                         self.logger.warning(
    #                             'ActualParmVFGNode (%d) does not contains necessary information', node.id)
    #                         return
    #                     callsite_nodes = self._vfg.search_nodes(
    #                         'ActualRetVFGNode', node.function, node.basic_block)
    #                     for callsite_node in callsite_nodes:
    #                         if callsite_node.ir is None:
    #                             self.logger.warning(
    #                                 'ActualRetVFGNode (%d) does not contains necessary information', callsite_node.id)
    #                             return
    #                         if match.group(1) in callsite_node.ir and not node.has_edge(node.name, callsite_node.name):
    #                             self._vfg.add_edge(
    #                                 Edge(node.name, callsite_node.name))
    #                             vfg_updated = True
    #                             self.logger.debug(
    #                                 "VFG updated when processing ActualParmVFGNode (%d)", node.id)
    #                 else:
    #                     node.label = node.ir
    #             case 'ActualRetVFGNode':
    #                 pattern = re.compile(r'(%\S+) = (call|invoke) .+ (@\S+)\((.+)\)')
    ...
    #                     if node.function is None or node.basic_block is None:
    #                         self.logger.warning(
    #                             'ActualRetVFGNode (%d) does not contains necessary information', node.id)
    #                         return
    #                     param_nodes = self._vfg.search_nodes(
    #                         'ActualParmVFGNode', node.function, node.basic_block)
    #                     for param_node in param_nodes:
    #                         for param_label in param_labels:
    #                             if param_label in param_node.ir and not node.has_edge(param_node.name, node.name):
    #                                 self._vfg.add_edge(
    #                                     Edge(param_node.name, node.name))
    #                                 vfg_updated = True
    #                                 self.logger.debug(
    #                                     "VFG updated when processing ActualRetVFGNode (%d)", node.id)
    #                 else:
    #                     node.label = node.ir
    #             case 'BinaryOPVFGNode':
    #                 pattern = re.compile(
    ...
    #                 assert node.lower_node_number == 1
    #                 pattern = re.compile(
    #                     r'(%\S+) = getelementptr inbounds (%\S+), (%\S+) (%\S+), (\S+) (\d+), (\S+) (\d+)')
    #                 match = pattern.match(node.ir)
    #                 if match:
    #                     element = match.group(1)
    #                     ptrvar = match.group(4)
    ...
    #         self.logger.info("SUB VFG scale: (node: %d, edge: %d)", subvfg.node_number, subvfg.edge_number)
    ...
    #     """
    #     edge_to_del = [edge for node in self._model for edge in node if not self._model.has_node_name(
    #         edge.target) or not self._model.has_node_name(edge.source)]
    ...
    #                         Edge(upper_node_name, lower_node_name))
    ...
    #                     upper_upper_node_name = next(
    #                         upper_node.upper_node_names)
    #                     self._model.add_edge(
    #                         Edge(upper_upper_node_name, node.name))
    ...
    #         if node.type == 'GepVFGNode':
    #             for lower_node_name in node.lower_node_names:
    #                 if self._model[lower_node_name].type == 'StoreVFGNode':
    #                     for edge in node:
    #                         edge.reverse()
    #                     break
...
if __name__ == "__main__":
    ...
    vfg = VFG.from_dot_file('examples/tf2/vfg.dot')
    logger.info("VFG scale: (node: %d, edge: %d)",
                vfg.node_number, vfg.edge_number)
    # vfg = Graph.from_dot_file('examples/tf2/full_svfg.dot')
    # logger.info("SVFG scale: (node: %d, edge: %d)", vfg.node_number, vfg.edge_number)
    node_ids = [1482, 77486, 77491, 1546, 1551, 77533, 77528]
    logger.info("Starting nodes: %s", node_ids)
    model = Model.extract(vfg, node_ids)
